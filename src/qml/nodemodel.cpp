...
void NodeModel::setVerificationProgress(double new_progress)
{
    if (new_progress != m_verification_progress) {
        ...
        setSyncTimeLeft(new_progress);
        Q_EMIT verificationProgressChanged();
    }
}

void NodeModel::setSyncTimeLeft(double new_progress)
{
    ...
    if (m_block_process_time.size() >= 2) {
        ...
        if(remainingMSecs >= 0) {
              m_sync_time = GUIUtil::formatNiceTimeOffset(remainingMSecs / 1000.0);
              Q_EMIT syncTimeLeftChanged();
        } else {
            m_sync_time = tr("unknown");
            Q_EMIT syncTimeLeftChanged();
        }
